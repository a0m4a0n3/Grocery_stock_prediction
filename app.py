...
if st.session_state.analyze:
    ...
    customer_type_counts = data_1['customer_type'].value_counts().reset_index()
    customer_type_counts.columns = ['customer_type', 'count']
    
    customer_type_chart = alt.Chart(customer_type_counts).mark_bar().encode(
        x=alt.X('customer_type', sort=None, axis=alt.Axis(labelAngle=0)),  # Horizontal labels
        y='count',
        color='customer_type'
    ).properties(width=600, height=400)
    
    st.altair_chart(customer_type_chart, use_container_width=True)

    st.subheader("Payment Type Analysis")
    
    # Bar chart for payment_type with horizontal labels
    payment_type_counts = data_1['payment_type'].value_counts().reset_index()
    payment_type_counts.columns = ['payment_type', 'count']
    
    payment_type_chart = alt.Chart(payment_type_counts).mark_bar().encode(
        x=alt.X('payment_type', sort=None, axis=alt.Axis(labelAngle=0)),  # Horizontal labels
        y='count',
        color='payment_type'
    ).properties(width=600, height=400)
    
    st.altair_chart(payment_type_chart, use_container_width=True)

    st.subheader("Sales by Category")

    # Calculate total sales per category
    total_sales_by_category = data_1.groupby('category')['total'].sum().reset_index()

    # Create a pie chart for total sales by category
    pie_chart = alt.Chart(total_sales_by_category).mark_arc().encode(
        theta=alt.Theta(field='total', type='quantitative'),
        color=alt.Color(field='category', type='nominal'),
        tooltip=['category', 'total']  # Tooltip to show category and total sales
    ).properties(width=400, height=400)

    st.altair_chart(pie_chart, use_container_width=True)
# Display the Google Colab link
st.markdown("[Click here to access the Colab notebook](https://colab.research.google.com/drive/1Pt9p0gqXRXN0s5u1JGyIfKANrZw8v-1M?usp=sharing)")
